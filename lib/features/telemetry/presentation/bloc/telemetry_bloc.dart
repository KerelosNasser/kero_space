...
class TelemetryBloc extends Bloc<bloc_event.TelemetryEvent, TelemetryState> {
  ...
  TelemetryBloc(
    ...
    _bindRepositoryWatchers();
    unawaited(_pruneData());
  }

  void _bindRepositoryWatchers() {
  ...
  void _scheduleDashboardRefresh() {
    ...
    _dashboardRefreshDebounce = Timer(const Duration(milliseconds: 1200), () {
      if (!isClosed) {
        add(const bloc_event.LoadTelemetryDashboard(silent: true));
      }
    });
  }
  ...
  Future<void> _onLoadDashboard(
    bloc_event.LoadTelemetryDashboard event,
    ...
    try {
      ...
      final results = await Future.wait<Object>([
        _screenRepo.getTotalScreenTimeMs(from: startOfDay, to: now),
        _usageRepo.getTodayUsage(),
        _usageRepo.getWeeklyScreenTimeTotals(),
        _blacklistRepo.getRules(),
        _platform.getAgentStatuses(),
      ]);
      ...
      emit(
        state.copyWith(
          ...
          clearError: true,
          todayScreenTimeMs: results[0] as int,
          todayTopApps: (results[1] as List<AppUsageRecord>).take(8).toList(),
          weeklyScreenTime: (results[2] as List).cast(),
          blacklistRules: (results[3] as List<BlacklistRule>),
          agentStatuses: (results[4] as Map<String, bool>),
        ),
      );
    } catch (_) {
      emit(
        state.copyWith(
          ...
          errorMessage: 'Failed to load telemetry data.',
        ),
      );
    } finally {
  ...
  Future<void> _onLoadHeatmap(
    bloc_event.LoadUnlockHeatmap event,
    Emitter<TelemetryState> emit,
  ) async {
    try {
      final heatmap = await _screenRepo.getUnlockHeatmap(
  ...
  Future<void> _onLoadBlockerStats(
    bloc_event.LoadBlockerStats event,
    Emitter<TelemetryState> emit,
  ) async {
    try {
      final now = DateTime.now();
      final startOfDay = DateTime(now.year, now.month, now.day);
      final blockerEvents = await _clickRepo.getEventsByName(
      ...
      for (final rule in rules) {
        ...
        var grantedOverrides = 0;

        for (final event in blockerEvents) {
          final data = jsonDecode(event.dataJson) as Map<String, dynamic>;
          if (data['packageName'] != rule.packageName) {
            continue;
          }
          final outcome = data['outcome'] as String? ?? '';
          if (outcome == 'blocked' || outcome == 'blocked_by_cooldown') {
            blockedAttempts++;
  ...
  Future<void> _onLoadClickLogs(
    bloc_event.LoadClickLogs event,
    Emitter<TelemetryState> emit,
  ) async {
    try {
      ...
      emit(
        state.copyWith(
          status: TelemetryStatus.failure,
          errorMessage: 'Failed to load click logs.',
        ),
      );
    }
  }
  ...
  Future<void> _onLoadBlacklist(
    bloc_event.LoadBlacklist event,
    Emitter<TelemetryState> emit,
  ) async {
    try {
      final rules = await _blacklistRepo.getRules();
      ...
      emit(
        state.copyWith(
          status: TelemetryStatus.failure,
          errorMessage: 'Failed to load blacklist.',
        ),
      );
    }
  }
  ...
  Future<void> _onAddRule(
    bloc_event.AddBlacklistRule event,
    Emitter<TelemetryState> emit,
  ) async {
    try {
  ...
  Future<void> _onRemoveRule(
    bloc_event.RemoveBlacklistRule event,
    Emitter<TelemetryState> emit,
  ) async {
    try {
  ...
  Future<void> _onUpdateRule(
    bloc_event.UpdateBlacklistRule event,
    Emitter<TelemetryState> emit,
  ) async {
    try {
  ...
  Future<void> _onToggleAgent(
    bloc_event.ToggleAgent event,
    Emitter<TelemetryState> emit,
  ) async {
    try {
      ...
      emit(
        state.copyWith(
          status: TelemetryStatus.failure,
          errorMessage: 'Failed to toggle agent.',
        ),
      );
    }
  }
  ...
  Future<void> _onRefreshStatuses(
    bloc_event.RefreshAgentStatuses event,
    Emitter<TelemetryState> emit,
  ) async {
    try {
      final statuses = await _platform.getAgentStatuses();
      ...
      emit(
        state.copyWith(
          status: TelemetryStatus.failure,
          errorMessage: 'Failed to refresh statuses.',
        ),
      );
    }
  }
